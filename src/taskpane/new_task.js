        ...
        success: function (resp) {
          localStorage.setItem("SelectedCompany", resp.data.data[0][0].companyfid);
            ...
            $.each(resp.data.data[0], function (data, val) {
              localStorage.setItem("SelectedCompany", resp.data.data[0][0].companyfid);
                var option = $("<option />");
                if (val.roletype.toLowerCase().includes("super"))
                    option.html(val.companyname + " (Super Admin)");
                else
                    option.html(val.companyname + " (" + val.roletype + ")");
  ...
  $.ajax({
      ...
      success: function (response, textStatus, xhr) {
        ...
        $.each(filterResult, function (data, value) {
            ddlBind1 += "<option value=\"" + value.ActivityID + '-' + value.SubActivityID + "\" data-id=\"" + value.ActivityID + "\">" + value.Name + "</option>";
        });
        //console.log(filterResult);
...
export async function CreateTask(newtask) {
    ...
    try {
      ...
      form.append("SchedulersData", "{\"frequencyID\":\"1\",\"frequencyCode\":\"O\",\"frequencyData\":\"{\\\"OonTime\\\":\\\"2022-09-26 06:57:00\\\"}\"}--{\"StartDate\":\"2022-09-26 06:57:00\",\"EndDate\":\"2022-12-26 06:57:00\",\"NoOfOcurrences\":null}");
      ...
      fetch(
        'https://dev.1viewtask.com/ComplianceMaster/CreateUpdateComplience',
        requestOptions,
      )
